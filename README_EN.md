# Dante Config Editor V3.5 - development branch

[Français](README.md) | **English**

V3.5 is developed and tested on the `v3.5` branch. Official V3.4.2 remains published on `main`.

**Stable version: [V3.4.2 Release for Windows and macOS](https://github.com/Mamat79/DanteConfigEditorV3/releases/tag/v3.4.2)**

> **Status: V3.5 in development. Unofficial third-party tool, not affiliated with Audinate.**
> This branch may still contain bugs. Always work on a copy and validate generated XML with official Dante tools.

**V3.5 documentation and presentation:**
...
The [V3.4.2 GitHub Release](https://github.com/Mamat79/DanteConfigEditorV3/releases/tag/v3.4.2) remains stable. V3.5 development packages are generated by the `Windows CI` and `macOS CI` runs for branch `v3.5`.

### Windows x64

Download artifact `DCE-v3.5-Windows-Installer`, containing `DanteConfigEditorV3_5_Installer.exe` and its SHA-256 checksum.

The self-contained installer includes the required .NET 8 runtime, French and English documentation, Start menu and desktop shortcuts, destination selection, and clean uninstall support. It installs by default in `C:\Program Files\Dante Config Editor V3.5\` and only upgrades an existing V3.5 installation, leaving stable V3.4.2 untouched.

### macOS

- `DanteConfigEditorV3_5_macOS_AppleSilicon.dmg` supports Apple Silicon Macs.
- `DanteConfigEditorV3_5_macOS_Intel.dmg` supports Intel 64-bit Macs.

Open the DMG and drag `Dante Config Editor V3.5` into `Applications`. The .NET runtime and both language manuals are included. Its separate bundle lets it coexist with V3.4.2.
...
Particular thanks to **[Tobias Grupe (@togrupe)](https://github.com/togrupe)** for his testing, detailed feedback, ideas, and contribution to the exchange workflow with **[dLive MIDI Tools](https://github.com/togrupe/dlive-midi-tools)**.
